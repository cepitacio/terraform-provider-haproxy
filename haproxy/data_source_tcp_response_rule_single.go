...
// TcpResponseRuleSingleDataSource defines the data source implementation.
type TcpResponseRuleSingleDataSource struct {
	client *HAProxyClient
}

// TcpResponseRuleSingleDataSourceModel describes the data source data model.
...
func (d *TcpResponseRuleSingleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if foundRule == nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("TCP response rule with index %d not found", data.Index.ValueInt64()))
		return
	}
