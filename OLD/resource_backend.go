...
// 		// Create a map of ACLs by name for efficient lookup
// 		stateAclsByName := make(map[string]backendAclResourceModel)
// 		for _, acl := range stateAcls {
// 			stateAclsByName[acl.AclName.ValueString()] = acl
// 		}

// 		// Track which ACLs we've processed
// 		processedAcls := make(map[string]bool)

// 		// Track ACLs that need to be recreated due to index changes
// 		var aclsToRecreate []backendAclResourceModel

// 		// First pass: identify ACLs that need index changes and mark them for recreation
// 		for index, planAcl := range planAclsMap {
// 			planAclName := planAcl.AclName.ValueString()
// 			stateAcl, ok := stateAclsByName[planAclName]
...
// 					return
// 				}
// 				processedAcls[planAclName] = true
// 			} else {
// 				// Check if the index has changed
// 				if stateAcl.Index.ValueInt64() != index {
// 					// Index has changed, mark for recreation
// 					log.Printf("ACL '%s' index changed from %d to %d, will recreate",
// 						planAclName, stateAcl.Index.ValueInt64(), index)
// 					aclsToRecreate = append(aclsToRecreate, planAcl)
...
// 					log.Printf("ACL '%s' at index %d is unchanged", planAclName, index)
// 				}
// 				processedAcls[planAclName] = true
// 			}
// 		}
...
// 		for _, planAcl := range aclsToRecreate {
// 			planAclName := planAcl.AclName.ValueString()
// 			stateAcl := stateAclsByName[planAclName]

// 			log.Printf("Deleting ACL '%s' at old index %d for recreation", planAclName, stateAcl.Index.ValueInt64())
// 			err := r.client.DeleteAcl(ctx, stateAcl.Index.ValueInt64(), "backend", plan.Name.ValueString())
// 			if err != nil {
// 				resp.Diagnostics.AddError(
// 					"Error deleting acl",
// 					fmt.Sprintf("Could not delete acl %s at old index %d, unexpected error: %s",
// 						planAcl.AclName.ValueString(), stateAcl.Index.ValueInt64(), err.Error()),
// 				)
// 				return
// 			}
// 		}
...
// 			log.Printf("Creating ACL '%s' at new index %d", planAclName, index)
// 			aclPayload := &ACLPayload{
// 				AclName:   planAcl.AclName.ValueString(),
// 				Index:     planAcl.Index.ValueInt64(),
// 				Criterion: planAcl.Criterion.ValueString(),
...
// 				resp.Diagnostics.AddError(
// 					"Error creating acl",
// 					fmt.Sprintf("Could not create acl %s at new index %d, unexpected error: %s",
// 						planAcl.AclName.ValueString(), index, err.Error()),
// 				)
// 				return
// 			}
// 		}
...
// 		for _, stateAcl := range stateAcls {
// 			if !processedAcls[stateAcl.AclName.ValueString()] {
// 				err := r.client.DeleteAcl(ctx, stateAcl.Index.ValueInt64(), "backend", plan.Name.ValueString())
// 				if err != nil {
// 					resp.Diagnostics.AddError(
// 						"Error deleting acl",
// 						fmt.Sprintf("Could not delete acl %d, unexpected error: %s", stateAcl.Index.ValueInt64(), err.Error()),
// 					)
// 					return
// 				}
// 			}
